...
func startHTTPServer(port string) {
	...
	handler := middleware.SecurityHeadersMiddleware(http.DefaultServeMux)
...
func startTLSServers(tlsManager *tlsmanager.TLSManager) {
	...
	go func() {
		httpsServer := &http.Server{
			...
			Handler:   middleware.SecurityHeadersMiddleware(http.DefaultServeMux), // Use default mux wrapped with security headers
		}
