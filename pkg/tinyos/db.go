...
func CreateDefaultUsers(db *sql.DB) error {
	...
	if count == 0 {
		...
		if envPass := os.Getenv("INITIAL_ADMIN_PASSWORD"); envPass != "" {
			password = envPass
			log.Printf("[INIT] Creating default user 'dyson' with provided password")
		} else {
			...
			password = hex.EncodeToString(tokenBytes)
			log.Printf("==================================================================")
			log.Printf("[INIT] Created default user 'dyson'")
			log.Printf("[INIT] GENERATED PASSWORD: %s", password)
			log.Printf("[INIT] Please change this password immediately using 'passwd'")
			log.Printf("==================================================================")
		}
