...
func CreateDefaultUsers(db *sql.DB) error {
	...
	if count == 0 {
		...
		if password == "" {
			...
			log.Printf("[SECURITY] Generated random password for 'dyson': %s (Please change this immediately!)", password)
		} else {
			log.Printf("[INIT] Using provided password for default user 'dyson'")
		}
